...
esp_err_sel_t selector_setAngle(int32_t i32SetAngleRaw, bool bInitTLV, uint32_t eSelNb)
{
	...
	if(s_i32ActAngleLast != i32SetAngleRaw)
	{
		...
		while((abs(s_i32ActAngleLast - i32SetAngleRaw) > 1))
		{
			...
			// ESP_LOGI("selector_setAngle", "SetAngle: %i ActAngle: %i DeltaAngle: %i Duty: %i Dir: %i", (int)i32SetAngle, (int)ui32ActAngle, (int)i32DeltaAngle, (int)i32Duty, eMotorDir);
...
esp_err_sel_t selector_setPos(int32_t i32SetPos)
{
	...
	return selector_setAngle(s_saSelProperties.ui32Angle[i32SetPos], false, 1);	//set angle for selector
}
...
//
esp_err_pump_t pump_runTime(uint32_t ui32Time, motor_direction_t eDirection, uint32_t ui32PumpNb)
{
	...
	{		
		...
		if ((counter%1)==0)
		{
			...
			int32_t i32SpeedKp = (int32_t)(0.05 * 1024);		//10Q10
			int32_t i32SpeedKi = (int32_t)(0.01 * 1024);		//10Q10;
...
esp_err_t erLevel_readPerc(int32_t *value)
{
	...
	ESP_LOGI("erLevel_readPerc","erLevelPerc: %d", (int)ui32LevelPerc);
